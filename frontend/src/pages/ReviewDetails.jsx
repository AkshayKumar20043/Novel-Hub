...
const ReviewDetails = () => {
    ...
    useEffect(() => {
        const fetchReviewDetails = async () => {
            try {
                const reviewResponse = await axios.get(`${API_URL}/novel-reviews/${novelId}/reviews/${reviewId}`, {
                    headers: {
    ...
    const handleReplySubmit = async () => {
        ...
        try {
            ...
            const updatedReview = await axios.get(`${API_URL}/novel-reviews/${novelId}/reviews/${reviewId}`, {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
            });
    ...
    if (error) return (
        <div className={`min-h-screen ${isDarkMode ? 'bg-gray-900 text-white' : 'bg-gray-50 text-gray-900'} flex justify-center items-center`}>
    ...
    return (
        <div className={`min-h-screen ${isDarkMode ? 'bg-gradient-to-br from-gray-900 to-black' : 'bg-gray-50'} transition-colors duration-300`}>
            <Navbar />
                ...
                >
                    <div className="p-8">
                        <div className="flex items-center gap-3 mb-8">
                            <FaFantasyFlightGames className={`text-3xl ${isDarkMode ? 'text-blue-400' : 'text-blue-600'}`} />
                        ...
                        <div className="space-y-6">
                            <div className="flex items-center gap-3">
                                <FaReply className={`text-2xl ${isDarkMode ? 'text-blue-400' : 'text-blue-600'}`} />
                            ...
                            <AnimatePresence>
                                {review.replies && review.replies.length > 0 ? (
                                    ...
                                    <motion.p
                                        initial={{ opacity: 0 }}
                                        animate={{ opacity: 1 }}
                                        className={`text-lg ${isDarkMode ? 'text-gray-400' : 'text-gray-600'} text-center py-8`}
                                    >
